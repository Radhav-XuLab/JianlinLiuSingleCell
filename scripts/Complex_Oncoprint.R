...
mat_ds = fread(file = "Oncogene matrix.csv",
              ...
              header = T, stringsAsFactors = F,
               data.table = F)
mat_ds = as.matrix(column_to_rownames(mat_ds, var = "V1"))
...
col = c("missense variant" = "darkgreen","frameshift variant" = "blue","splice acceptor variant" = "purple","splice donor variant" = "black","inframe deletion" = "red","stop gained" = "orange")
n_mutated_patients = apply(mat, 1, function(x) sum(x != ""))
n_mutated_genes = apply(mat, 2, function(x) sum(x != ""))
ht1 = oncoPrint(mat, name = "cases",alter_fun = alter_fun, get_type = function(x) strsplit(x, ";")[[1]],col = col,column_order = NULL,show_column_names = T,column_title = " ",
...
ha_cn2 = HeatmapAnnotation(cn = anno_text(colnames(mat_ps), rot = -50, just = "left",offset = unit(1.35, "npc") - unit(1, "mm"), gp = gpar(fontsize = 8,fontface = "bold")), annotation_height = unit(6, "cm"))
ht2 = Heatmap(mat_ps, col = c("0" = "white", "1" = "purple"), 
        ...
        show_heatmap_legend = FALSE, width = unit(9, "cm"), column_title = " ")

#Heatmap for annotation (Third heatmap)
ht3 = Heatmap(mat_ds, col = c("0" = "white", "1" = "deeppink","2" = "dodgerblue4", "4" = "limegreen","3" = "brown","5" = "tomato"), 
        rect_gp = gpar(col = "black"), show_row_names = FALSE, cluster_columns = FALSE,
        show_column_dend = FALSE,  show_column_names = FALSE,heatmap_legend_param = list(title = "Annotation",at = c("1","2","3","4","5"),labels = c("ONCO","PRE-ONCO","TSG","PRE-TSG","Cancer gene census")),
        show_heatmap_legend = T, width = unit(1.75, "cm"), column_title = " ")


#Draw the heatmap in a file
png(filename="plot.png", width = 22000, height = 8000, res = 900)
...
decorate_heatmap_body("cases", {
  i = which(colnames(mat) == "VMMG17")
  x = i/ncol(mat)
  grid.lines(c(x, x), c(0, 1), gp = gpar(fill="white", col = "white",lwd = 3))
  grid.text("VMMG", x, unit(1, "npc") + unit(1, "mm"),hjust=2,vjust = -2,gp=gpar(fontsize=20,fontface = "bold"))
})

decorate_heatmap_body("cases", {
  i = which(colnames(mat) == "476MMG9")
  x = i/ncol(mat)
  grid.lines(c(x, x), c(0, 1), gp = gpar(fill="white", col = "white",lwd = 3))
  grid.text("476MMG", x, unit(1, "npc") + unit(1, "mm"),hjust=1.1,vjust = -2,gp=gpar(fontsize=20,fontface = "bold"))
})

decorate_heatmap_body("cases", {
  i = which(colnames(mat) == "476PT20")
  x = i/ncol(mat)
  grid.lines(c(x, x), c(0, 1), gp = gpar(fill="white", col = "white",lwd = 3))
  grid.text("476PT", x, unit(1, "npc") + unit(1, "mm"),hjust=2.5,vjust = -2,gp=gpar(fontsize=20,fontface = "bold"))
})
...
decorate_heatmap_body("cases", {
  ...
  x = i/ncol(mat)
  grid.lines(c(x, x), c(0, 1), gp = gpar(fill="white", col = "white",lwd = 3))
  grid.text("153PT", x, unit(1, "npc") + unit(1, "mm"),hjust=2,vjust = -2,gp=gpar(fontsize=20,fontface = "bold"))
})

decorate_heatmap_body("cases", {
  i = which(colnames(mat) == "153LMT20")
  x = i/ncol(mat)
  grid.lines(c(x, x), c(0, 1), gp = gpar(fill="white", col = "white",lwd = 3))
  grid.text("153LMT", x, unit(1, "npc") + unit(1, "mm"),hjust=2,vjust = -2,gp=gpar(fontsize=20,fontface = "bold"))
})
